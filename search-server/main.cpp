...
void TestAddDocFindDoc()
{
...

    {
        ...
        server.AddDocument(0, "белый кот и модный ошейник"s, DocumentStatus::ACTUAL, {8, -3});
        ASSERT_EQUAL(server.FindTopDocuments("белый кот и модный ошейник").size(), 1u);
    }
}
...
void TestMatch()
{
    ...
    const auto match_docs1 = search_server.MatchDocument("пушистый ухоженный кот"s, 0);
    const auto match_docs2 = search_server.MatchDocument("пушистый ухоженный кот"s, 1);
    const auto match_docs3 = search_server.MatchDocument("пушистый ухоженный кот"s, 2);
    const auto match_docs4 = search_server.MatchDocument("пушистый ухоженный кот"s, 3);

    ASSERT_EQUAL(std::get<0>(match_docs1).size(), 1u);
    ASSERT_EQUAL(std::get<0>(match_docs1).at(0), "кот"s);
    ASSERT(std::get<1>(match_docs1) == DocumentStatus::ACTUAL);

    ASSERT_EQUAL(std::get<0>(match_docs2).size(), 2u);
    ASSERT_EQUAL(std::get<0>(match_docs2).at(0), "кот"s);
    ASSERT_EQUAL(std::get<0>(match_docs2).at(1), "пушистый"s);
    ASSERT(std::get<1>(match_docs2) == DocumentStatus::BANNED);

    ASSERT_EQUAL(std::get<0>(match_docs3).size(), 1u);
    ASSERT_EQUAL(std::get<0>(match_docs3).at(0), "ухоженный"s);
    ASSERT(std::get<1>(match_docs3) == DocumentStatus::IRRELEVANT);

    ASSERT_EQUAL(std::get<0>(match_docs4).size(), 1u);
    ASSERT_EQUAL(std::get<0>(match_docs4).at(0), "ухоженный"s);
...
void TestRaiting()
{
    ...
    {
        SearchServer server;
        server.AddDocument(0, "белый кот и модный ошейник"s, DocumentStatus::ACTUAL, {-9});
...
void TestStatus()
{
    ...
    const auto found_docs2 = search_server.FindTopDocuments("пушистый ухоженный кот"s,
                                                            [](int document_id, DocumentStatus status, int rating)
                                                            {
    ...
    const auto found_docs3 = search_server.FindTopDocuments("пушистый ухоженный кот"s,
                                                            [](int document_id, DocumentStatus status, int rating)
                                                            {
    ...
    const auto found_docs4 = search_server.FindTopDocuments("пушистый ухоженный кот"s,
                                                            [](int document_id, DocumentStatus status, int rating)
                                                            {
